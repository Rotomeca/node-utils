import { EMPTY_STRING, SPACE, UI_ZERO } from "./constants";
import { MayBe, toUint, uint } from "./types";
...
/**
 * Remplace les occurrences `{key}` dans une chaîne par les valeurs correspondantes.
 *
 * Les clés absentes du dictionnaire `vars` sont remplacées par une chaîne vide.
 * Les valeurs numériques sont converties en chaîne automatiquement.
 *
 * @param str - La chaîne contenant les marqueurs `{key}` à remplacer.
 * @param vars - Un dictionnaire associant chaque clé à sa valeur de substitution.
 * @returns La chaîne avec tous les marqueurs remplacés.
 *
 * @example
 * ```ts
 * template("Bonjour {prenom} !", { prenom: "Alice" })      // → "Bonjour Alice !"
 * template("{a} + {b} = {c}", { a: 1, b: 2, c: 3 })       // → "1 + 2 = 3"
 * template("Valeur : {x}", {})                             // → "Valeur : "
 * template("Pas de marqueur", { foo: "bar" })              // → "Pas de marqueur"
 * ```
 */
export function template(
  ...
  return str.replace(
    /\{(\w+)\}/g,
    (_, key) => vars[key]?.toString?.() ?? EMPTY_STRING,
  );
}

/**
 * Compte le nombre d'occurrences non chevauchantes de `sub` dans `str`.
 *
 * @param str - La chaîne dans laquelle rechercher.
 * @param sub - La sous-chaîne à compter.
 * @returns Le nombre d'occurrences de `sub` dans `str`, ou `0` si `sub` est vide.
 *
 * @remarks
 * Les occurrences chevauchantes ne sont pas comptées.
 * Une sous-chaîne vide (`sub = ""`) retourne systématiquement `0`.
 *
 * @example
 * ```ts
 * countOccurrences("abcabc", "a")   // → 2
 * countOccurrences("aaa", "aa")     // → 1  (non chevauchant)
 * countOccurrences("abc", "x")      // → 0
 * countOccurrences("abc", "")       // → 0
 * countOccurrences("", "a")         // → 0
 * ```
 */
export function countOccurrences(str: string, sub: string): uint {
  if (sub.length === 0) return UI_ZERO;
  return toUint(str.split(sub).length - 1);
}

/**
 * Retourne la chaîne avec ses caractères dans l'ordre inverse.
 *
 * Utilise `Intl.Segmenter` avec une granularité graphème pour gérer
 * correctement les emojis composés (ZWJ sequences), les paires de
 * substitution (surrogate pairs) et les caractères combinés.
 *
 * Si `Intl.Segmenter` n'est pas disponible dans l'environnement, un fallback
 * basé sur l'itérateur de string est utilisé — les surrogate pairs sont alors
 * correctement gérés, mais pas les ZWJ sequences ni les caractères combinés.
 *
 * @param str - La chaîne à inverser.
 * @param options - Options de segmentation.
 * @param options.locales - Locale utilisée par `Intl.Segmenter` (défaut : `"fr-FR"`).
...
export function reverse(
  str: string,
  { locales = "fr-FR" }: { locales?: Intl.LocalesArgument } = {},
): string {
  if (typeof Intl?.Segmenter === "undefined")
    return [...str].reverse().join(EMPTY_STRING);
...
/**
 * Découpe une chaîne en mots en gérant intelligemment camelCase, PascalCase,
 * snake_case, kebab-case et les espaces.
 *
 * Délègue la normalisation à {@link toSnakeCase} puis découpe sur `_`.
 *
 * @param str - La chaîne à découper.
 * @returns Un tableau de mots en minuscules.
 *
 * @example
 * ```ts
 * words("helloWorld")         // → ["hello", "world"]
 * words("Bonjour Le Monde")   // → ["bonjour", "le", "monde"]
 * words("foo-bar")            // → ["foo", "bar"]
 * words("foo_bar")            // → ["foo", "bar"]
 * words("__foo__bar__")       // → ["foo", "bar"]
 * words("")                   // → []
 * ```
 */
